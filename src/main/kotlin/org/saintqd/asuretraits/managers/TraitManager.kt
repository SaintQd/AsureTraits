...
class TraitManager {
    ...
    val traits = hashMapOf<String, AsureTrait>()
    ...
    fun loadParams(plugin : Plugin) {
        ...
        val filePaths = AsureUtils.listFilesInFolder(plugin.dataFolder.path + File.separator + "Traits")
        for (filePath in filePaths) {
            ...
            for (traitName in config.getKeys(false)) {
                ...
                if (!traitName.matches(TRAIT_NAME_REGEX)) {
                    AsureTraits.inst().logger.warning { "Traits: Trait name $traitName does not match regex: $TRAIT_NAME_REGEX! Trait won't be loaded." }
                    continue
                }
                ...
                if (traitActionFunction == null) {
                    AsureTraits.inst().logger.warning { "Traits: There is no action type of $type! Is it registered? Trait $traitName won't be loaded." }
                    continue
                }

                val traitAction = registerAction(traitName,traitConfig,traitActionFunction)
                val trait = AsureTrait(traitName.lowercase(), display, lore,model, cost,traitAction,
                ...
                if (config.getBoolean("$traitName.RegisterAsPlaceholder")) {
                    AsureTraitsPlaceholders.instance?.let { instance ->
        ...
        bindHintTask?.cancel()
        val bindHintPeriod = AsureTraits.inst().config.getLong("Traits.BindHintPeriod",6000L)
        if (bindHintPeriod > 0) {
            Bukkit.getScheduler().runTaskTimer(AsureTraits.inst(), Runnable {
                    ...
                    for (traitName in traitOwner.traits) {
                        ...
                        if (trait.action is BindableAction && trait.action.isBindable()) {
                            if (!traitOwner.bindedTraits.contains(traitName)) {
                                traitOwner.player.sendMessage(LangManager.INSTANCE.parseLangString(AsureTraits.inst(),"binded_traits_hint_message"))
                                break
                            }
                        }
                    }
    ...
    fun executeAction(actionName : String, traitOwner: TraitOwner) : Boolean {
        if (!actionsRegistry.contains(actionName)) {
            AsureTraits.inst().logger.warning { "Trait action $actionName does not exist." }
        }
    ...
    fun registerActionType(actionName : String, function : (String, ConfigurationSection) -> TraitAction) {
        if (AsureTraits.inst().loadFinished) {
            AsureTraits.inst().logger.severe("Could not register trait action $actionName: Registering only allowed during server loading phase.")
            return
        }
        else {
