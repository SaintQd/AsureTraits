package org.saintqd.asuretraits.gui
...
class TraitsGUI(player: Player) : AsureGUI(player) {
    ...
    fun setTraitMenu(page : Int = 1) {
        ...
        val isSimpleLayout = AsureTraits.inst().config.getBoolean("Gui.Simple",false)

        var availablePoints = AsureTraits.inst().config.getInt("Traits.AvailablePoints",10)
        ...
        inventory = Bukkit.createInventory(
            ...
            AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "trait_gui_title",availablePoints.toString()))
        buttons.clear()
        ...
        for (trait in TraitManager.instance.traits.values) {
            ...
            loreLines.add(AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "trait_gui_select_trait"))
            traitIcon.setData(DataComponentTypes.LORE, ItemLore.lore(loreLines))

            buttons[slotIndex] = AsureGUIButton().consumer { _ ->
                player.sendMessage(AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "trait_preselect",trait.displayName))
                ...
                val soundName = AsureTraits.inst().config.getString(
                    "Gui.Sounds.click","")!!
        ...
        if (page > 1) {
            ...
            val modelName = AsureTraits.inst().config.getString("Gui.Models.PrevPage", "paper")!!
            pageItem.setData(DataComponentTypes.ITEM_MODEL, Key.key(modelName))
            pageItem.setData(
                DataComponentTypes.CUSTOM_NAME,
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_prev_page")
            )
            pageItem.setData(DataComponentTypes.LORE,ItemLore.lore().addLine(
                        AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_prev_page_lore")
                    ).build()
            )
        ...
        if (biggerAmountOfTraits > page * maxTraitsPerPage) {
            val pageItem = ItemStack.of(Material.PAPER)
            val modelName = AsureTraits.inst().config.getString("Gui.Models.NextPage", "paper")!!
            pageItem.setData(DataComponentTypes.ITEM_MODEL, Key.key(modelName))
            pageItem.setData(
                DataComponentTypes.CUSTOM_NAME,
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_next_page")
            )
            pageItem.setData<ItemLore?>(
                DataComponentTypes.LORE,
                ItemLore.lore().addLine(
                        AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_next_page_lore")
                    )
                    .build()
            )
        ...
        var modelName = AsureTraits.inst().config.getString("Gui.Models.Close", "paper")!!
        closeItem.setData(DataComponentTypes.ITEM_MODEL, Key.key(modelName))
        closeItem.setData(DataComponentTypes.CUSTOM_NAME,
            AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_close")
        )
        closeItem.setData(DataComponentTypes.LORE,ItemLore.lore()
                .addLine(AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_close_lore"))
                .build()
        )

        val closeButton = AsureGUIButton().consumer { _: InventoryClickEvent ->
        ...
        modelName = AsureTraits.inst().config.getString("Gui.Models.Review", "green_concrete")!!
        if (traitOwner.preselectedTraits.isNotEmpty())
            modelName = AsureTraits.inst().config.getString("Gui.Models.ReviewPending", "lime_concrete")!!
        reviewItem.setData(DataComponentTypes.ITEM_MODEL, Key.key(modelName))
        reviewItem.setData(DataComponentTypes.CUSTOM_NAME,
            AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_review")
        )
        val loreBuilder = ItemLore.lore()
        for (line in AsureLib.inst().langManager.langLines[NamespacedKey.fromString("asuretraits:menu_review_lore")]!!.split("<newline>")) {
            loreBuilder.addLine { AsureUtils.parseString(line) }
        }
        reviewItem.setData(DataComponentTypes.LORE,loreBuilder.build())

        val reviewButton = AsureGUIButton().consumer { _: InventoryClickEvent ->
            setReviewMenu()
            player.openInventory(inventory)
    ...
    fun setReviewMenu(page: Int = 1) {
        ...
        inventory = Bukkit.createInventory(
            AsureGUIHolder(this), size,
            AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "trait_review_gui_title",availablePoints.toString()))
        buttons.clear()
        ...
        for (trait in sortedList) {
            ...
            if (canBind) {
                ...
                var interactionHintLine = LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "menu_review_bind_trait_interaction_hint")
                    .replace("{1}","1")
                ...
                for (interactionType in InteractionType.entries) {
                    if (selectedInteractionType != null && interactionType == selectedInteractionType) {
                        interactionTypesString.add(LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "menu_review_bind_trait_interaction_selected")
                            .replace("{1}","<key:${interactionType.key}>"))
                    }
                    else {
                        interactionTypesString.add(LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "menu_review_bind_trait_interaction_not_selected")
                            .replace("{1}","<key:${interactionType.key}>"))
                    }
                }
                interactionHintLine += interactionTypesString.joinToString(", ")
                loreLines.add(AsureUtils.parseString(interactionHintLine))

                var sneakingLine = LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "menu_review_bind_trait_require_sneaking_hint")
                    .replace("{1}","2").replace("{2}",
                        LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "text_disabled"))
                if (bindData != null && bindData.requireSneaking) {
                    sneakingLine = LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "menu_review_bind_trait_require_sneaking_hint")
                        .replace("{1}","2").replace("{2}",
                            LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "text_enabled"))
                }
                loreLines.add(AsureUtils.parseString(sneakingLine))

                var emptyHandLine = LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "menu_review_bind_trait_require_empty_hand_hint")
                    .replace("{1}","3").replace("{2}",
                        LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "text_disabled"))
                if (bindData != null && bindData.requireEmptyHand) {
                    emptyHandLine = LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "menu_review_bind_trait_require_empty_hand_hint")
                        .replace("{1}","3").replace("{2}",
                            LangManager.INSTANCE.getRawLangString(AsureTraits.inst(), "text_enabled"))
                }
                loreLines.add(AsureUtils.parseString(emptyHandLine))
                loreLines.add(LangManager.INSTANCE.parseLangString(AsureTraits.inst(),"menu_review_bind_trait_remove"))
            }
            ...
            val button = AsureGUIButton().consumer { event: InventoryClickEvent ->
                ...
                if (event.click.isRightClick && canBind) {
                    ...
                    traitOwner.player.sendMessage(LangManager.INSTANCE.parseLangString(AsureTraits.inst(),"menu_review_bind_trait_remove_message",trait.displayName))
        ...
        if (traitOwner.preselectedTraits.isNotEmpty() || traitOwner.preselectedTraitsToRemove.isNotEmpty()) {
            ...
            val traitChangeCooldown = AsureTraits.inst().config.getLong("Traits.SecondsBetweenChange",2592000L)
            var canConfirm = true

            val loreBuilder = ItemLore.lore()

            if (!traitOwner.player.hasPermission("asuretraits.selectcooldownbypass")) {
                ...
                if ((traitOwner.lastTraitChangeTimestamp + traitChangeCooldown) > Clock.System.now().epochSeconds) {
                    ...
                    if (possibleApplyCooldown) {
                        ...
                        canConfirm = false
                        loreBuilder.addLine { AsureLib.inst().langManager.parseLangString(AsureTraits.inst(),"trait_gui_select_trait_cooldown",timeToCooldown.toString()) }
                    }
                }
            }
            ...
            if (availablePoints < 0) {
                canConfirm = false
                loreBuilder.addLine { AsureLib.inst().langManager.parseLangString(AsureTraits.inst(),"trait_gui_select_trait_cooldown",availablePointsFormat) }
                loreBuilder.addLine { AsureLib.inst().langManager.parseLangString(AsureTraits.inst(),"trait_gui_select_trait_no_points") }
            }

            val confirmItem = ItemStack.of(Material.LIME_CONCRETE)
            val modelName = AsureTraits.inst().config.getString("Gui.Models.ReviewConfirm", "lime_concrete")!!
            confirmItem.setData(DataComponentTypes.ITEM_MODEL, Key.key(modelName))
            confirmItem.setData(
                DataComponentTypes.CUSTOM_NAME,
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_confirm")
            )
            if (canConfirm) {
                loreBuilder.addLine { AsureLib.inst().langManager.parseLangString(AsureTraits.inst(),"trait_gui_select_trait_cooldown",availablePointsFormat) }
                for (line in AsureLib.inst().langManager.langLines[NamespacedKey.fromString("asuretraits:menu_confirm_lore_hint")]!!.split("<newline>")) {
                    loreBuilder.addLine { AsureUtils.parseString(line) }
                }
                var possibleApplyCooldown = false
                for (traitName in traitOwner.preselectedTraitsToRemove) {
                ...
                if (possibleApplyCooldown) {
                    for (line in AsureLib.inst().langManager.langLines[NamespacedKey.fromString("asuretraits:menu_confirm_lore")]!!.split("<newline>")) {
                        loreBuilder.addLine { AsureUtils.parseString(line.replace("{1}", (traitChangeCooldown / 86400).coerceAtLeast(1).toString())) }
                    }
                }
                val button = AsureGUIButton().consumer { _: InventoryClickEvent ->
                    for (traitName in traitOwner.preselectedTraitsToRemove) {
                        if (!traitOwner.traits.contains(traitName)) {
                            player.sendMessage { AsureLib.inst().langManager.parseLangString(AsureTraits.inst(),"menu_confirm_preselected_trait_to_remove_not_present") }
                    ...
                    player.sendMessage { AsureLib.inst().langManager.parseLangString(AsureTraits.inst(),"menu_confirm_success") }
        ...
        if (traitOwner.preselectedTraits.isNotEmpty() || traitOwner.preselectedTraitsToRemove.isNotEmpty()) {
            ...
            val modelName = AsureTraits.inst().config.getString("Gui.Models.Reset", "paper")!!
            reviewItem.setData(DataComponentTypes.ITEM_MODEL, Key.key(modelName))
            reviewItem.setData(
                DataComponentTypes.CUSTOM_NAME,
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_reset")
            )
            val loreBuilder = ItemLore.lore()
            for (line in AsureLib.inst().langManager.langLines[NamespacedKey.fromString("asuretraits:menu_reset_lore")]!!.split("<newline>")) {
                loreBuilder.addLine { AsureUtils.parseString(line) }
            }
            ...
            val button = AsureGUIButton().consumer { _: InventoryClickEvent ->
                ...
                player.sendMessage { AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_reset_success") }
        ...
        val modelName = AsureTraits.inst().config.getString("Gui.Models.Return", "barrier")!!
        returnItem.setData(DataComponentTypes.ITEM_MODEL, Key.key(modelName))
        returnItem.setData(DataComponentTypes.CUSTOM_NAME,
            AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_return")
        )

        val returnButton = AsureGUIButton().consumer { _: InventoryClickEvent ->
            setTraitMenu()
            player.openInventory(inventory)
    ...
    @Suppress("UnstableApiUsage")
    private fun generateTraitItemStack(trait: TraitManager.AsureTrait, ownerLinkedTraits : Set<String>) : ItemStack {

        val traitIcon = ItemStack.of(Material.STONE)

        val costIdentifier = if (trait.cost > 0)
            "asuretraits:trait_gui_positive_cost_format"
        else if (trait.cost < 0)
            "asuretraits:trait_gui_negative_cost_format"
        else
            "asuretraits:trait_gui_neutral_cost_format"
        val absCost = trait.cost.absoluteValue.toString()
        val langLine = AsureLib.inst().langManager.langLines[NamespacedKey.fromString(costIdentifier)]!!.replace("{1}",absCost)
        traitIcon.setData(DataComponentTypes.CUSTOM_NAME, AsureUtils.parseString("${trait.displayName} $langLine"))

        val loreLines = AsureUtils.parseStringList(trait.lore)
        val itemLoreBuilder = ItemLore.lore()
        if (!trait.canDisable || ownerLinkedTraits.contains(trait.name))
            itemLoreBuilder.addLine { AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "menu_review_remove_deny_lore") }
