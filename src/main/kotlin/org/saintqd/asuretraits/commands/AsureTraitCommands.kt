package org.saintqd.asuretraits.commands
...
class AsureTraitCommands {
    ...
    companion object {
                    ...
                    Commands.literal("asuretraits")
                        ...
                        .then(
                            Commands.literal("reload")
                                .requires { predicate: CommandSourceStack ->
                                    predicate.sender.hasPermission("asuretraits.admin")
                                }
                        ...
                        .then(
                            Commands.literal("menu")
                                .requires { predicate: CommandSourceStack ->
                                    predicate.sender.hasPermission("asuretraits.menu")
                                }
                            .executes { ctx: CommandContext<CommandSourceStack> ->
                                val ctxSource = ctx.getSource()
                                openMenuCommand(
                        ...
                        .then(
                            Commands.literal("reviewmenu")
                                .requires { predicate: CommandSourceStack ->
                                    predicate.sender.hasPermission("asuretraits.reviewmenu")
                                }
                            .executes { ctx: CommandContext<CommandSourceStack> ->
                                val ctxSource = ctx.getSource()
                        ...
                        .then(
                            Commands.literal("use")
                                .requires { predicate: CommandSourceStack ->
                                    predicate.sender is Player &&
                                    predicate.sender.hasPermission("asuretraits.use")
                                }
                                .then(
                        ...
                        .then(
                            Commands.literal("toggleinteract")
                                .requires { predicate: CommandSourceStack ->
                                    predicate.sender.hasPermission("asuretraits.toggleinteract")
                                }
                                .executes { ctx: CommandContext<CommandSourceStack> ->
                        ...
                        .then(
                            Commands.literal("togglebindhint")
                                .requires { predicate: CommandSourceStack ->
                                    predicate.sender.hasPermission("asuretraits.togglebindhint")
                                }
                                .executes { ctx: CommandContext<CommandSourceStack> ->
        ...
        private fun reloadCommand(sender: CommandSender) {
            AsureTraits.inst().loadData()
            if (sender is Player) sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "reload_message")
            )
        }

        private fun saveDataCommand(sender: CommandSender) {
            AsureTraits.inst().saveData()
            if (sender is Player) sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "save_data_message")
            )
        }
        ...
        private fun addTraitCommand(sender: CommandSender, traitName : String, player: Player?) {
            ...
            if (trait == null) {
                sender.sendMessage(
                    AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_trait_does_not_exist",traitName)
                )
                return
            }
            ...
            if (traitOwner == null) {
                sender.sendMessage(
                    AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_trait_owner_not_exist",player.name))
                return
            }
            if (traitOwner.traits.contains(trait.name)) {
                sender.sendMessage(AsureLib.inst().langManager.parseLangString(AsureTraits.inst(),
                    "command_add_trait_already_present",traitName,player.name))
                return
            }
            traitOwner.addTrait(trait)
            sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_add_trait_success",traitName,player.name))
            return
        }
        ...
        private fun addPreselectedTraitCommand(sender: CommandSender, traitName : String, player: Player?) {
            ...
            sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_add_preselected_trait_success",traitName,player.name))
            return
        }
        ...
        private fun addPreselectedTraitToRemoveCommand(sender: CommandSender, traitName : String, player: Player?) {
            ...
            if (traitName == "ALL") {
                ...
                sender.sendMessage(
                    AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_add_preselected_trait_to_remove_success",traitName,player.name))
                return
            }
            else {
                val trait = TraitManager.instance.traits[traitName]
        ...
        private fun removeTraitCommand(sender: CommandSender, traitName : String, player: Player?) {
            ...
            sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_remove_trait_success",traitName,player.name))
            return
        }
        ...
        private fun removePreselectedTraitCommand(sender: CommandSender, traitName : String, player: Player?) {
            ...
            traitOwner.preselectedTraits.remove(trait.name)
            sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_remove_preselected_trait_success",traitName,player.name))
            return
        }
        ...
        private fun resetPreselectedTraitsCommand(sender: CommandSender, player: Player?) {
            ...
            sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_reset_preselected_traits_success",player.name))
            return
        }
        ...
        private fun listPlayerTraitsCommand(sender: CommandSender, player: Player) {
            ...
            sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "list_player_traits_title",player.name))
            traitOwner.traits.forEachIndexed { index, traitName ->
                sender.sendMessage(
                    AsureLib.inst().langManager.parseLangString(AsureTraits.inst(),
        ...
        private fun listOnlinePlayersWithTraitCommand(sender: CommandSender, traitName : String) {
            ...
            sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "list_online_players_with_trait_title",traitName))
        ...
        private fun resetSelectCooldownCommand(sender: CommandSender, playerName: String?) {
            ...
            if (uuid != null) {
                ...
                if (traitOwner != null) {
                    ...
                    sender.sendMessage(
                        AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_reset_select_success",traitOwner.player.name))
                }
                else {
                    AsureTraits.inst().storage?.resetTraitOwnerSelectCooldown(uuid)
                    sender.sendMessage(
                        AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_reset_select_success",uuid.toString()))
                }
            }
        ...
        private fun removeTraitOwnerDataCommand(sender: CommandSender, playerName: String, force : Boolean) {
            ...
            if (traitOwner != null) {
                ...
                traitOwner.lastTraitChangeTimestamp = 0L
                sender.sendMessage(
                    AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_remove_data_success",traitOwner.player.name))
            }
            else {
                AsureTraits.inst().storage?.removeTraitOwnerData(uuid,force)
                sender.sendMessage(
                    AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_remove_data_success",uuid.toString()))
            }
        ...
        private fun resetActionCooldownsCommand(sender: CommandSender, player: Player?) {
            ...
            sender.sendMessage(
                AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_reset_action_success",player.name))
        ...
        private fun useTraitActionCommand(sender: CommandSender, traitName : String, player: Player?) {
            ...
            if (!trait.executableViaCommand) {
                sender.sendMessage(
                    AsureLib.inst().langManager.parseLangString(AsureTraits.inst(), "command_use_trait_not_bindable",traitName)
                )
                return
            }
            val traitOwner = TraitManager.instance.traitOwners[player.uniqueId]
        ...
        private fun toggleInteractCommand(sender: CommandSender, state: String?, player: Player?) {
            var player = player
            player = AsureUtils.checkForPlayerPresent(sender, player)
            val vaultManager = AsureLib.inst().vaultManager

            val boolState: Boolean
            if (state == null) {
                val triState = player.permissionValue("asuretraits.interactdisabled")
                boolState = triState != TriState.TRUE
            } else boolState = state.toBoolean()

            if (boolState) {
                vaultManager.permissionProvider.playerAdd(null, player, "asuretraits.interactdisabled")
                if (sender === player) sender.sendMessage(
                    AsureLib.inst().langManager
                        .parseLangString(AsureTraits.inst(), "command_toggle_interact_false")
                )
                else sender.sendMessage(
                    AsureLib.inst().langManager
                        .parseLangString(AsureTraits.inst(), "command_toggle_interact_false_other", player.name)
                )
            } else {
                vaultManager.permissionProvider.playerRemove(null, player, "asuretraits.interactdisabled")
                if (sender === player) sender.sendMessage(
                    AsureLib.inst().langManager
                        .parseLangString(AsureTraits.inst(), "command_toggle_interact_true")
                )
                else sender.sendMessage(
                    AsureLib.inst().langManager
                        .parseLangString(AsureTraits.inst(), "command_toggle_interact_true_other", player.name)
                )
            }
        }
        ...
        private fun toggleBindHintCommand(sender: CommandSender, state: String?, player: Player?) {
            ...
            val boolState: Boolean
            if (state == null) {
                val triState = player.permissionValue("asuretraits.bindhintdisabled")
                boolState = triState != TriState.TRUE
            } else boolState = state.toBoolean()

            if (boolState) {
                vaultManager.permissionProvider.playerAdd(null, player, "asuretraits.bindhintdisabled")
                if (sender === player) sender.sendMessage(
                    AsureLib.inst().langManager
                        .parseLangString(AsureTraits.inst(), "command_binded_traits_toggle_disable")
                )
                else sender.sendMessage(
                    AsureLib.inst().langManager
                        .parseLangString(AsureTraits.inst(), "command_binded_traits_toggle_disable_other", player.name)
                )
            } else {
                vaultManager.permissionProvider.playerRemove(null, player, "asuretraits.bindhintdisabled")
                if (sender === player) sender.sendMessage(
                    AsureLib.inst().langManager
                        .parseLangString(AsureTraits.inst(), "command_binded_traits_toggle_enable")
                )
                else sender.sendMessage(
                    AsureLib.inst().langManager
                        .parseLangString(AsureTraits.inst(), "command_binded_traits_toggle_enable_other", player.name)
                )
            }
        }
    }
