...
@AsureTraitType("hide_in_container")
    ...
    companion object {
        ...
        private val listener = object : Listener {
            ...
            fun onContainerInteract(event : PlayerInteractEvent) {
                ...
                if (cooldown.getOrDefault(event.player, AsureUtils.getCurrentTick()) > AsureUtils.getCurrentTick()) {
                    event.setCancelled(true)
                    return
                }
                if (containerBlocks.containsKey(event.clickedBlock)) {
                    ...
                    if (foundPlayer != null && playerData != null && foundPlayer.isValid && foundPlayer.isOnline) {
                        ...
                        event.player.sendMessage(AsureLib.inst().langManager.parseLangString(
                            AsureTraits.inst(), "hide_in_containers_message",foundPlayer.name))
                        event.setCancelled(true)
            ...
            fun onPlayerMove(event: PlayerMoveEvent) {
                hiddenPlayers[event.player]?.let { playerData ->
                    if (playerData.player.isSneaking && playerData.hiddenUntil <= AsureUtils.getCurrentTick()) {
                        removePlayer(playerData)
                    } else {
                        val to = event.to
                        val from = event.from
        ...
        private fun removePlayer(playerData: HiddenPlayerData) {
            ...
            cooldown[playerData.player] = AsureUtils.getCurrentTick() + 10
