...
@AsureTraitType("mm_skill_on_potion_effect")
    ...
    init {
        ...
        for (potionEffectKey in possiblePotionEffects) {
            try {
                ...
                AsureTraits.inst().logger.warning("Traits: Trait $name: There is no PotionEffectType named ${potionEffectKey.lowercase()}")
            }
        }
        ...
        for (causeKey in possibleCauses) {
            ...
            if (possibleCause.isPresent) {
                ...
                AsureTraits.inst().logger.warning("Traits: Trait $name: There is no EntityPotionEffectEvent Cause named ${causeKey.uppercase()}")
        }
    }
