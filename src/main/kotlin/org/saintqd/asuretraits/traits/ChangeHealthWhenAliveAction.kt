...
@AsureTraitType("change_health_when_alive")
class ChangeHealthWhenAliveAction(name : String, config : ConfigurationSection) : TraitAction(name,config) {
    ...
    override fun register() {
        if (actionsMap.isEmpty()) {
            ...
            healthCheckTask = Bukkit.getScheduler().runTaskTimer(AsureTraits.inst(), Runnable {
    ...
    private fun setAbsorption(player : Player) {
        ...
        if (AsureTraits.inst().cmiEnabled) {
        ...
        if (diffAmount == 0L) {
            ...
            Bukkit.getScheduler().runTaskLater(AsureTraits.inst(), Runnable {
    ...
    companion object {
        ...
        val ACTION_KEY = NamespacedKey(AsureTraits.inst(),"change_health_when_alive")
        val TIME_SINCE_DEATH_KEY = NamespacedKey(AsureTraits.inst(),"change_health_when_alive_last_death")
        val SAVED_HEALTH_KEY = NamespacedKey(AsureTraits.inst(),"change_health_when_alive_saved_health")
